...
onAuthStateChanged(auth, user => {
  // ── Keep track of who's currently signed in, in this exact order,
  //    BEFORE anything touches local storage — every local read/write
  //    of project data is namespaced by this uid so one account can
  //    never see or overwrite another account's files on this device.
...
window.doSignOut = async function () {
  ...
  if (typeof window.closeProjectSync === 'function') window.closeProjectSync();

  // ── Full reload (not just a UI reset) ──
  // The editor keeps the open project's files in memory (and a live
  // Firestore listener) for the whole tab session. If we just hid the
  // auth screen and let the next person log in in the same tab, they
  // could briefly see, or even autosave over, the previous account's
  // project before the app finished loading their own. Reloading the
  // page guarantees every in-memory trace of this session is gone
  // before the next login starts.
  sessionStorage.setItem('_justSignedOut', '1');
  location.reload();
};

// ── Show the "Signed out" toast once, right after the post-sign-out reload ──
document.addEventListener('DOMContentLoaded', () => {
  if (sessionStorage.getItem('_justSignedOut')) {
    sessionStorage.removeItem('_justSignedOut');
    setTimeout(() => { if (typeof showToast === 'function') showToast('Signed out', 'info', 'fa-right-from-bracket'); }, 300);
  }
});
